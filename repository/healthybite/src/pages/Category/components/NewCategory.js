...
function NewCategory({handleUpdate, setAddCategory, setAddFood, foods, platesData}) {
    ...
    return (
        ...
        {/* Loader de Pantalla Completa (o al menos del componente) */}
        {loading ? (
            <div className='w-full h-full flex flex-col justify-center items-center py-10'>
                <FontAwesomeIcon icon={faSpinner} spin className='text-white text-4xl mb-3' />
                <p className='text-white font-bold'>Adding category...</p>
            </div>
        ) : (
            <>
                {message &&
                ...
                <div className='flex flex-col w-full mt-3 '>
                    ...
                    <div className='flex flex-row w-full justify-center items-center mt-2'>
                        {/* Se mantiene `handleSingleClick` pero ahora internamente comprueba el estado `loading` */}
                        <SaveButton label="Save new category" handleChanges={handleSingleClick} disabled={loading}/>
                    </div> 
                </div>
            </>
        )}
